...
function startConsole() {
  ...
  clearChatBtn.addEventListener('click',()=>{ if(!confirm('Clear chat history?')) return; messagesRef.remove(); chatMessages.innerHTML=''; });
  nickBtn.addEventListener('click', ()=>{ nickInput.value=myNick; nickModal.style.display='block'; nickInput.focus(); });
  nickSave.addEventListener('click', ()=>{ const newNick = nickInput.value.trim(); if(newNick){ myNick=newNick; localStorage.setItem('reze_chat_nick', myNick); } nickModal.style.display='none'; });
  nickCancel.addEventListener('click', ()=>{ nickModal.style.display='none'; });
  nickInput.addEventListener('keydown', e => { if(e.key==='Enter'){ e.preventDefault(); nickSave.click(); } if(e.key==='Escape'){ e.preventDefault(); nickCancel.click(); } });
  ...
  document.addEventListener('keydown', e => {
    ...
    if(e.key==='Backspace') input=input.slice(0,-1);
      ...
      else if(cmd==='sysinfo'){ append('[SYSINFO]'); append('  User Agent: '+navigator.userAgent); append('  Screen: '+screen.width+'x'+screen.height); append('  Language: '+navigator.language); append('  Platform: '+(navigator.platform||'unknown')); }
