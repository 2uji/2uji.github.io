...
function startConsole() {
  ...
  const render = () => {
    consoleOverlay.innerHTML = text + prompt + input + '<span class="cursor">█</span>';
    consoleOverlay.scrollTop = consoleOverlay.scrollHeight;
  };
  const append = line => {
    if (line !== undefined && line !== null && line !== false) text += line + '\n';
    render();
  };
  ...
  async function loadCloud() {
    ...
    try {
      ...
      append('[ERROR] Failed to connect to cloud.');
    }
  }
  ...
  nickSave.addEventListener('click', () => {
    ...
    if (newNick) {
      myNick = newNick;
      localStorage.setItem('reze_chat_nick', myNick);
    }
  ...
  const boot = setInterval(() => {
    append(bootLines[idx]);
    if (++idx >= bootLines.length) { clearInterval(boot); append(); }
  }, 800);
